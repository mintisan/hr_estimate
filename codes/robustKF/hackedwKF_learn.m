%************************************************************
% File:         hackedwKF_learn.m
% Date:         January 8, 2008
% Author:       Jo-Anne Ting
% Description:
%  Implements the "hacked" weighted Kalman filter (as
...
function  [x, weight, S, P, A, C, Q, R, ss] = ...
          hackedwKF_learn(x_prev, z, P, A, C, Q, R, ss)
...
S = C*(A*oldP*A' + Q)*C' + R/(weight+SMALL);
P = inv(weight*(C'/(R+SMALL))*C + 1/(A*oldP*A' + Q) + SMALL);
x = P/(A*P*A' + Q)*A*x_prev  + weight*P*C'/(R+SMALL)*z;
